...
// Type-only, so the SiteHeader <-> MobileHeader import cycle is erased at
// compile time and never exists in the bundle.
import type { HeaderCta } from "./SiteHeader";
...
export function MobileHeader({
  ...
  cta?: HeaderCta;
  /** href of the NAV item for the current page — see SiteHeader. */
  active?: string;
}) {
    ...
    >
      ...
      <div className="relative">
        <header className="grid grid-cols-[auto_1fr_auto] items-center gap-3 overflow-hidden rounded-2xl bg-[#181818] px-4 py-3">
          ...
          {/* Violet call button where the webapp keeps its avatar — or, when
              a light CTA override is passed (the /templates Login), a white
              disc with a person glyph. Both invert their own colours. */}
          <a
            href={cta?.href ?? BOOKING_URL}
            aria-label={cta?.label ?? "Book a call"}
            className={`grid h-12 w-12 place-items-center rounded-[12px] transition-all duration-150 ${
              cta?.variant === "light"
                ? "bg-[linear-gradient(140deg,#fdfdfd_0%,#ececec_100%)] text-black hover:bg-[linear-gradient(140deg,#000_0%,#000_100%)] hover:text-white active:bg-[linear-gradient(140deg,#000_0%,#000_100%)] active:text-white"
                : "bg-[linear-gradient(140deg,#a08ade_8%,#7c54b5_42%,#6e54b5_93%)] text-white hover:bg-[linear-gradient(140deg,#fff_0%,#fff_100%)] hover:text-[#6e54b5] active:bg-[linear-gradient(140deg,#fff_0%,#fff_100%)] active:text-[#6e54b5]"
            }`}
          >
            {cta?.variant === "light" ? (
              <svg
                ...
                fill="none"
                strokeWidth="2"
                className="size-[24px] stroke-current"
                aria-hidden="true"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M20 21a8 8 0 0 0-16 0M12 13a4.5 4.5 0 1 0 0-9 4.5 4.5 0 0 0 0 9Z"
                />
              </svg>
            ) : (
            <svg viewBox="0 0 23 23" fill="none" strokeWidth="2" className="size-[24px] stroke-current" aria-hidden="true">
          ...
          >
            {NAV.map((item) => (
              <a
                ...
                // The current page's row keeps the hover fill permanently.
